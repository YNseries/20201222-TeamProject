...
public class MovieSystem {
	...
	Movie[] movieAllData()
	{
		...
		return m;   // 배열 넘길 땐 배열 명만 넣으면 됨
	}
	// 사용자 검색
	Movie[] movieFindData(String ss)    // 인스턴스 메소드
	{
		int count=0;
		for(String s:title)
		{
			if(s.contains(ss))
			{
				count++;   // 배열이 몇 개 생성해야하는지 모르기 때문에 확인하기 위함
			}
		}
		// 배열 초기화
		...
		for(int j=0;j<50;j++)
		{
			if(title[j].contains(ss))    // 찾은 위치인 j에서 갖고와야 함
			{
				m[i]=new Movie();
				m[i].title=title[j];
				m[i].singer=singer[j];
