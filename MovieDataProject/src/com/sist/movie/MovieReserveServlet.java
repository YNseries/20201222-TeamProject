...
public class MovieReserveServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 웹 프로그램은 데이터를 다른 클래스로 전송할 때, 항상 ?를 이용해서 데이터를 전송한다(매개변수 x)
		// ?키=값
		// ?키=값&키=값 ==> HTML은 값을 받을 수 없다 => 자바 이용: request.getParameter("키")
		// HTML: 화면만 출력, CSS: 화면 디자인 => 정적 페이지 => 자바 System.out.println()
		 ...
		 *       ==============
		 *       C(Controller): Model+View => 서블릿으로 되어있음(스프링이 지원함)
		 *                                                 ====== 이번엔 사용(x), 직접 제작했음
		 *       서블릿명?키=값  => 앞에 있는 서블릿 클래스에서 받아서 처리
