 ...
 *    인터페이스 여러개 ==> A,B,C
 *    =======
 *     주로 => 관련된 여러개의 클래스를 한 개의 이름으로 제어
 */
public class MovieMainFrame222 extends JFrame implements ActionListener,Runnable{
    ...
    public MovieMainFrame222()
    {
    	...
    	setDefaultCloseOperation(DO_NOTHING_ON_CLOSE);  // x 눌러도 안닫힘
    	...
    	// 채팅
    	cf.tf.addActionListener(this);  // Enter 누를 때
    	// 쪽지 보내기
     ...
     *       case 2:
     *       {
     *         String name="";   // 같은 변수이기 때문에 {} 없으면 오류남
     *       }
     *         break;
     *    }
     */
    public void run()
    {
    	try
    	{
    		while(true)
    		{
    			...
    			StringTokenizer st=new StringTokenizer(msg,"|");  // "|"는 일반 문자열
    			// split => 정규식 문자열 "\\|"
    			// Function.LOGIN+"|"+id+"|"+name+"|"+sex => 테이블에 출력
    			// Function.MYLOG => 윈도우 변경(로그인 => 채팅창)
    			// Function.CHAT => text area(ta)에 출력
    			...
    			switch(protocol)
    			{
	    			...
	    			{
	    				String id=st.nextToken();  // Server.class에서 종료하고 남기게 될 id
	    				String str="";
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if(e.getSource()==cf.tf)  // enter 눌렀을 때
		{
			// 1. 입력한 채팅 문자열 읽기
			String msg=cf.tf.getText();  // cf 채팅폼 / tf textField
			if(msg.length()<1)
				return;
			
			String color=cf.box.getSelectedItem().toString();
			// Object => String형 변경: .toString()
			//                        (String)
			try
			{
				out.write((Function.CHAT+"|"+msg+"|"+color+"\n").getBytes());  // 서버로 전송
			}catch(Exception ex) {ex.printStackTrace();}
			cf.tf.setText("");  // 끝나고 초기화시킴
			cf.box.setSelectedIndex(0);  // 검정색으로 바꿈
		}
		else if(e.getSource()==cf.b3)  // 나가기 버튼
		{
			try
			{
				out.write((Function.EXIT+"|\n").getBytes());  // 서버로 전송 => 서버에서 처리해야 함
				// readLine() => 패킷을 받을 때 \n
				// 코딩은 클라이언트    =========>    서버    =========>    클라이언트
				//         요청(Enter, 버튼/메뉴 클릭)      요청 처리(결과값)       결과값을 출력   
			}catch(Exception ex) {ex.printStackTrace();}
		}
