...
public class ViewDAO {
	...
	public ViewDAO()
	{
		try
		{
			...
			// 메소드 제오, 멤버변수, 생성자, 매개변수를 제어할 수 있다
...
// 기능(목록 출력) => SQL문장, VIEW를 이용
	...
	public void empAllData()
	{
		try
		{
			...
			*/
			...
			// 5. 결과값 출력 => ResultSet => 단위가 Recored => empno,ename,job,hiredate,sal,dname,loc,grade
			while(rs.next())
			{
	...
	public void empDetailData(int empno)
	{
		try
		{
			...
			*/
			String sql="SELECT * FROM emp_dept_view "
					+ "WHERE empno=?";
			// View => SELECT의 모든 기능(GROUP BY, WHERE, ORDER BY)을 사용할 수 있다
			// View도 일반 테이블처럼 사용된다
			...
			rs.next();
			// 4. 결과값 출력 => 브라우저면, VO에 값을 채운다(하나만 출력하면 되니까 while 없어도 됨)
			System.out.println(rs.getInt(1)+" "
					+rs.getString(2)+" "
					+rs.getString(3)+" "
					+rs.getDate(4)+" "
	...
	public void movieListData(int page)  // 사용자가 보내준 페이지를 받아서 해당 페이지만 전송
	{
		try
		{
			...
			// ?에 값 채우기
			int rowSize=20;
			int start=(page*rowSize)-(rowSize-1);   // rownum은 1번부터 시작이라 -1 해 줘야 함
			int end=rowSize*page;
			/*
			 * 	1page: 1~20
			 *  2page: 21~40   ==> rownum은 1번부터 시작
			 *  ...
			 *  ...
			 */
			ps.setInt(1, start);
