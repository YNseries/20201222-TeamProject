import java.util.Scanner;   // import는 항상 class 위에!
/*
 *    ==================
 *    변수 모이면 => 배열
 *    명령문 모이면 => 메소드     =====> 클래스 (여러개를 다시 묶는다(관련된 클래스끼리)) ==> 패키지
 *    ====
 *    연산자 + 제어문
 *    ==================
 */
public class 메소드활용_4 {  //전체적으로 쓸 때
	static String[] title={           // class 안에 있어야만 다른 곳에서 갖고와 쓸 수 있음(멤버메소드) / main 안에 있으면 못 가져다 씀
	         	"Your Lights",
				"겨울잠",
				"우린 어쩌다 헤어진 걸까",
				"요즘",
	...
	public static void main(String[] args) {  // 메인 블록 안에서만 쓸 때
		// TODO Auto-generated method stub
		// 1. 전체 목록 출력 => for-each(못씀)
		/* System.out.println("===== 뮤직 Top50 전체 목록 =====");
		for(int i=0;i<50;i++)  // 50곡
		{
		...
		for(int i=0;i<50;i++)
		{
			if(title[i].contains(ss))  // 일부 문자를 검색해도 그 문자가 포함된 모든 결과가 나옴  
		             // 문자열 안에 포함 여부 확인 contains => boolean(있으면 true/없으면 false)로 나옴
				     //	=> 오라클에서는 (LIKE), REGEXP_LIKE(), 정규식
			{
				System.out.println("순위:"+(i+1));
				System.out.println("노래명:"+title[i]);
				System.out.println("가수명:"+singer[i]);
		...
		String s=scan.next();  // 여기에 s 대신 singer 쓰면 배열과 충돌해서 안됨
		for(int i=0;i<50;i++)
		{
			if(singer[i].contains(s))           // 검색은 여기서 하고(가수명 일부가 포함된)
				      // contains 대신 startsWith 써도 됨. 이건 시작하는 문자가 같은지?
			{
				System.out.println(title[i]);   // 인덱스번호 같이 해서 검색한 가수의 노래 제목이 출력됨
			}
		}
	}
